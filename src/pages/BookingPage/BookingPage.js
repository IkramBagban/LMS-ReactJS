...
function BookingPage() {
  ...
  const showitem = (item) => {
    return (<div key={item.itemID + Math.random} className="col-md-6" style={{ margin: '10px 0px 10px 0px', padding: '0' }}>
      <Card ide={item.itemID}
      //  img1={item.image_url}
       img1={dummyImage}
        header={item.item_name} price={item.pricing} category={item.item_cat1} emirateNumber={emirateNumber} deliveryType={selectedDeliveryType} />
    </div>)
          ...
          <div className="row" style={{ marginTop: '0px', paddingTop: '0px', marginBottom: '0' }}>
            ...
            {!openSearch ? (filteredData.length > 0 ? (
              ...
              filteredData.map((item) => (
                <div key={item.itemID + Math.random()} className="col-md-6" style={{ margin: '10px 0px 10px 0px', padding: '0' }}>
                  <Card ide={item.itemID}
                  //  img1={item.image_url} 
                   img1={dummyImage} 
                   header={item.item_name} price={item.pricing} category={item.item_cat1} emirateNumber={emirateNumber} deliveryType={selectedDeliveryType} />
