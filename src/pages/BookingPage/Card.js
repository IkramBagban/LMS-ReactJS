...
function Card({ ide, img1, header, price, emirateNumber, deliveryType }) {
  ...
  /*  const prevQuantity=useSelector(state =>
      state.cart.items.find(item => item.ide === ide)?.quantity || 0
    );
    const prevService=useSelector(state =>
      state.cart.items.find(item => item.ide === ide)?.selectedService || 0
    );
    const prevDelivery=useSelector(state =>
      state.cart.items.find(item => item.ide === ide)?.Delivery || 0
    );*/
  const { quantity: prevQuantity, selectedService: prevService } = useSelector(
    (state) =>
      state.cart.items.find((item) => item.ide === ide) || {
        quantity: 0,
  ...
  // // const { quantity: prevQuantity, selectedService: prevService }
  // const prevQuantity = item.quantity || 0;
  // const prevService = item.selectedService || "Dryclean"
  ...
  const selectedPrices = currentPrice
    ?.filter(
      (obj) =>
  ...
  const finalprice = (
    parseFloat(selectedPrices[0]).toFixed(2) * parseFloat(quantity)
  ).toFixed(2);
              ...
              <div className={classes.check}>
                <label className={classes.label}>
                  <input
                    ...
                    checked={selectedService === "Dryclean" && quantity > 0}
                  />
                  Dry clean
                </label>
                <label className={classes.label}>
                  <input
                    ...
                    value="Laundry"
                    onChange={handleRadioChange}
                    checked={selectedService === "Laundry" && quantity > 0}
                  />
                  Laundry
                </label>
                <label className={classes.label}>
                  <input
                    ...
                    value="Press"
                    onChange={handleRadioChange}
                    checked={selectedService === "Press" && quantity > 0}
                  />
                  Press
                </label>
              </div>
              ...
              <div className={classes.check}>
                <label className={classes.label}>
                  <input
                    ...
                    checked={foldedChecked && quantity > 0}
                  />
                  Folded
                </label>
                <label className={classes.label}>
            ...
            <div className={`  ${classes.qty} col`}>
              <div className={`${classes.check} text-center`}>
                <label>
                  AED{" "}
                  {(
                    parseFloat(selectedPrices[0]).toFixed(2) *
                    parseFloat(quantity)
                  ).toFixed(2)}
                </label>
              </div>
