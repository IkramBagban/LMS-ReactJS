...
const getOrders = async () => {
  try {
    ...
    const response = await axios.get(`${API_URL}/orders/659fd037814c3d2d47337600`);
...
const postSignUp = async(data)=>{
  try{
    ...
    const response = await axios.post("http://localhost:9090/auth/signup",data)
...
const  putProfile = async(updatedData)=> {
  try{
    ...
    const response = await axios.put(`${API_URL}/auth/update/659fd037814c3d2d47337600`, updatedData)
...
export {getCustomers, getProducts, getServices, getDeliveryTypes,getEmirates,getArea, postOrder,getOrders, postSignUp,putProfile,postOTP, VerifyOTP , resetPassword };
